          ...
          {services.map(({ id, title, description, icon: Icon, cta }) => (
            ...
            >
              <div className="flex h-full flex-col rounded-2xl border border-neutral-800 bg-neutral-900 p-8 shadow-lg transition hover:ring-2 hover:ring-orange-500 dark:border-neutral-300 dark:bg-neutral-800">
                ...
                <p className="flex-grow text-sm dark:text-neutral-300 text-neutral-700">
                  {description}
                </p>
                <Link href={cta.href} className="mt-6 inline-block">
                  <button className="hover:cursor-pointer w-full rounded-lg bg-gradient-to-r from-red-600 to-orange-500 py-3 font-semibold text-white transition hover:to-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:ring-offset-2">
                    {cta.label}
                  </button>
                </Link>
              </div>
